...
## 미입력 진행 선생님 선택 일괄 지정
- 학습 기록 보완의 `실제 진행 선생님 미입력` 모드에서 staff는 현재 페이지의 보완 가능한 행을 체크하거나 전체 선택할 수 있다. 상단 선생님 선택 후 확인창에서 학생·대상 날짜·도서·기록 번호·개수를 확인하고 일괄 저장한다. 일반 선생님 및 수업 내용 모드에는 선택 도구를 표시하지 않는다.
- `POST /api/user/studylog-completion/bulk-teacher`는 `{teacher_username, records:[{row_id, token}]}`을 받는다. 1~50개의 서로 다른 양의 정수 rowid만 허용하며 기존 빈칸·토큰·권한·학생 식별·정산 제한을 재검증한다. 모든 변경과 개별 UPDATE 감사는 한 트랜잭션으로 처리한다.
- 학생·항목·페이지 변경 또는 재조회 시 선택을 해제한다. 저장 성공 시 선택한 행의 초안만 지우고 나머지 입력은 유지한다. 실패 시 선택을 해제하고 새로고침 전까지 일괄 재시도를 막는다. 개별 지정·저장 기능은 유지한다.


## 작은 화면의 검색 필터·목록 높이
- `compact-search-view`는 도서·학생·학습 기록·수업·활동 이력·변경 이력·결제 이력·월급 정산의 조회 화면에만 적용한다. 카드 패딩·필터 간격·제목 여백을 줄이되 등록 폼과 설정 화면의 공용 스타일은 유지한다.
