...
- 시작 시점 동작(`init_system_tables()`): (a) `_app_users` 인증 테이블 생성 + **admin 계정만 시드**(manager/teacher 기본 계정은 시드하지 않음 — 배포 시 admin만 존재, staff는 admin이 계정 관리 UI에서 발급), (b) 수업용 `Classes`/`ClassStudents` 테이블 생성(기존 DB면 `ClassStudents.IsSpecial` 컬럼을 ALTER로 보완), (c) `StudyLogs`에 앱 전용 컬럼(`LessonContent`, `Description`, `IsSpecial`)이 없으면 `ALTER TABLE ... ADD COLUMN`으로 보완.
